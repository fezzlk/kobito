...
- kobito自身のリポジトリ（`~/repos/kobito`）や作業対象リポジトリでスクリプト・テストを実行した場合、生成された一時ファイル・キャッシュ（`__pycache__`等、コミット対象でないもの）は、その実行を終える前に削除する。無人実行のため、誰も気づかずに残り続けることを防ぐ。
...
保留中の依頼があれば内容に従う（例: 特定issueの優先、一時停止の指示など）。ただし、特定issueの優先・再開指示も「3. 着手可否の判断」を省略または上書きしない。拾った依頼は着手時に`complete`で取り除く。
...
- 承認あり（LINEボタン、または明確な承認の意思を示すコメント）: 先に「3-2. 通常の着手可否判断」を現在のissue内容で再実施する。対象外条件に該当しなければ、ラベルを`kobito:in-progress`に戻し、statusを`researching`へ更新し、対応する`user-to-agent`エントリがあれば`complete`した上で「5. 作業」に進む（計画は既に`agent-to-user`とLinearコメントへ書いた内容のまま進めてよく、あらためて4を経由する必要はない）。実機・現物確認を含む対象外条件に該当する場合は、承認済みでも再開せず、人間が扱うissueとして残す。
...
- 完了条件・受け入れ条件・必要な調査や検証に、実機・現物・実環境での人手確認が含まれていないか。以下はプロジェクトを問わず**kobitoの自律作業対象外**とし、コード変更や自動テストだけを先行できる場合もissue全体をclaimしない。
  - スマートフォン、タブレット、カメラ、マイク、センサー、Bluetooth、位置情報など、物理デバイスでの動作確認
  - 撮影品質、認識精度、向き、照明、音声、操作感、通知到達など、実物を見たり操作したりしないと合否を判定できない確認
  - 店舗、ネットワーク機器、外部端末、第三者アカウントなど、kobitoから直接扱えない現物・実環境での確認
  - ユーザーによる手動確認を経なければ完了できない作業

  エミュレーター、シミュレーター、モック、自動テストで一部を再現できても、最終的な完了判定に実機・現物確認が残る場合は対象外とする。調査中や実装中に必要性が判明した場合も、その時点で作業を中断する。単に判断待ちとして通知して再開を勧めるのではなく、人間が扱うissueとして残し、kobitoから推薦・催促しない。
